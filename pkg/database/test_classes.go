...
func (db *Database) GetTestClassByTeachersID(teacherID int32) (*pgx.Rows, error) {
	return db.conn.Query("SELECT test_class_name, test_class_data FROM test_classes WHERE teacher_id = $1", teacherID)
}
...
func (db *Database) AddTeachersTestClass(teacherID int32, name string, video string) (*pgx.Rows, error) {
	return db.conn.Query("INSERT INTO test_classes(teacher_id,test_class_name, test_class_data) VALUES($1,$2,$3) RETURNING test_class_name, test_class_data", teacherID, name, video)
}
...
func (db *Database) UpdateTeachersTestClass(teacherID int32, name string, video string) (*pgx.Rows, error) {
	return db.conn.Query("UPDATE test_classes SET test_class_name = $2, test_class_data = $3 WHERE teacher_id = $1 RETURNING test_class_name, test_class_data", teacherID, name, video)
}
