...
type DBManagement struct {
	...
	Months      pgtype.Int4        `json:"months"`
...
func (db *Database) UpdateUserManagement(managementID int32, job string, institution string, months int32) (pgx.Rows, error) {
...
func (db *Database) DeleteUserManagement(managementID int32) (pgx.Rows, error) {
	return db.conn.Query(context.Background(), "DELETE FROM users_management WHERE user_management_id=$1", managementID)
}
